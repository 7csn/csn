...
class Csn
{
    ...
    // 文件引入相关信息
    protected static $file = [__CSN__ => 'csn', App => 'app', Web => 'web'];
    // 自动加载类相关信息
    protected static $class = ['app\models\\' => App, 'app\\' => App, 'csn\t\\' => Csn_t, 'csn\y\\' => Csn_y, 'csn\\' => Csn];
    ...
    static function obj($class, array $args = [], $type = '')
    {
        ...
        $class = ($type === 'm' ? 'app\\models\\' : 'csn\\' . ($type === '' ? '' : $type . '\\')) . $class;
    ...
    static function load($class)
    {
        ...
        $type = $res[0];
        $name = strtolower(str_replace('\\', XG, str_replace($type, '', $class)));
        $file = $res[1] . $name;
